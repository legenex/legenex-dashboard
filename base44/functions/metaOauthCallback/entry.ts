...
Deno.serve(async (req) => {
  ...
  const redirectBack = (extra: string) =>
    new Response(null, {
      status: 302,
      headers: { Location: `${settingsUrl}${extra}` },
    });
  ...
  try {
    ...
    if (existing) {
      await base44.asServiceRole.entities.IntegrationConfig.update(existing.id, { config: configString });
    } else {
      await base44.asServiceRole.entities.IntegrationConfig.create({ name: 'meta', config: configString });
    }
