...
function KnowledgeBaseTab() {
  ...
  const { data: docs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.KnowledgeDoc.list('sort_order'),
  });
  ...
  const save = async () => {
    ...
    const { id, ...rest } = form;
    if (id) await base44.entities.KnowledgeDoc.update(id, rest);
    else await base44.entities.KnowledgeDoc.create(rest);
  ...
  const remove = async (d) => {
    await base44.entities.KnowledgeDoc.delete(d.id);
    qc.invalidateQueries({ queryKey: ['knowledge-docs'] });
    toast.success('Removed');
  };

  const toggle = async (d) => {
    await base44.entities.KnowledgeDoc.update(d.id, { active: !d.active });
    qc.invalidateQueries({ queryKey: ['knowledge-docs'] });
  };
...
function ConversationsTab() {
  ...
  const { data: conversations = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ChatConversation.filter({ active: true }, '-last_message_at', 50),
  });

  const archive = async (c) => {
    await base44.entities.ChatConversation.update(c.id, { active: false });
...
function MemoriesTab() {
  ...
  const { data: memories = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ChatMemory.filter({ active: true }, '-created_date', 100),
  });

  const dismiss = async (m) => {
    await base44.entities.ChatMemory.update(m.id, { active: false });
