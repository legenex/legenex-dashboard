...
export default function SheetSourceDialog({ open, onOpenChange, source, onSaved }) {
  ...
  const loadColumns = async () => {
    ...
    setBusy(true);
    try {
      const res = await base44.functions.invoke('syncGoogleSheets', { preview: true, sheet_id: sheetId, worksheet: form.worksheet });
  ...
  const save = async () => {
    ...
    try {
      ...
      await base44.functions.invoke('provisionLeadSource', { source_id: saved.id });
