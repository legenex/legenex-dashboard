...
export default function LeadSourcesPanel() {
  ...
  const { data: sources = [] } = useQuery({ queryKey: ['lead-sources'], queryFn: () => base44.entities.LeadSource.list('-created_date', 100) });
  ...
  const syncNow = async (s) => {
    setSyncing(s.id);
    try {
      const res = await base44.functions.invoke('syncGoogleSheets', { source_id: s.id });
  ...
  const toggleEnabled = async (s) => {
    await base44.entities.LeadSource.update(s.id, { enabled: !s.enabled });
    refresh();
  };

  const remove = async (s) => {
    await base44.entities.LeadSource.delete(s.id);
